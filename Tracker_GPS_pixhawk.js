...
function parseMavFromDrone(mavPacket) {
    try {
        ...
        if (msg_id == mavlink.MAVLINK_MSG_ID_GLOBAL_POSITION_INT) { // #33
            ...
            globalpositionint_msg.lat = Buffer.from(lat, 'hex').readInt32LE(0);
            globalpositionint_msg.lon = Buffer.from(lon, 'hex').readInt32LE(0);
            globalpositionint_msg.alt = Buffer.from(alt, 'hex').readInt32LE(0);
            globalpositionint_msg.relative_alt = Buffer.from(relative_alt, 'hex').readInt32LE(0);
            globalpositionint_msg.hdg = Buffer.from(hdg, 'hex').readUInt16LE(0);
            ...
            attitude_msg.roll = Buffer.from(roll, 'hex').readFloatLE(0);
            attitude_msg.pitch = Buffer.from(pitch, 'hex').readFloatLE(0);
            attitude_msg.yaw = Buffer.from(yaw, 'hex').readFloatLE(0);
